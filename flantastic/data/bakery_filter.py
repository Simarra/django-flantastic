...
TMP_FILE = TMP_DIR + os.sep + 'tmp_estabs.csv.gz'
...
for gm_chunk in pd.read_csv(TMP_FILE, chunksize=CHUNK_SIZE, compression=COMPRESSION):
    tmp_res = gm_chunk[gm_chunk[NAF_COL].isin(NAF_CODES)]
    tmp_res.enseigne.fillna(tmp_res.l1_normalisee, inplace=True)
    tmp_res = tmp_res[COLS_TO_KEEP]
    chunks.append(tmp_res)

result = pd.concat(chunks)
result.to_csv('myres.csv')
